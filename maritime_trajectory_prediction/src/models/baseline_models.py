...
class TrajectoryLSTM(nn.Module):
    ...
    def __init__(
        ...
        self.attention = nn.MultiheadAttention(
            embed_dim=lstm_output_dim, num_heads=8, dropout=dropout, batch_first=True
        )
        ...
        logger.info(
            f"Initialized TrajectoryLSTM: input_dim={input_dim}, hidden_dim={hidden_dim}, "
...
class AnomalyAutoencoder(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized AnomalyAutoencoder: input_dim={input_dim}, "
    ...
    def forward(self, x: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if len(original_shape) == 3:
            ...
            encoding = encoding.view(batch_size, seq_len, -1).mean(
                dim=1
    ...
    def compute_anomaly_score(self, x: torch.Tensor) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            if len(mse_error.shape) == 3:  # Sequence data
                anomaly_scores = mse_error.mean(
                    dim=(1, 2)
                )  # Average over seq and features
            else:
...
class VesselGCN(nn.Module):
    ...
    def __init__(
        ...
        logger.info(
            f"Initialized VesselGCN: node_features={node_features}, "
    ...
    def forward(
        ...
        # Apply GCN layers
        for i, (gcn_layer, layer_norm) in enumerate(
            zip(self.gcn_layers, self.layer_norms, strict=False)
        ):
            # GCN forward pass
    ...
    def predict_interactions(
        ...
        with torch.no_grad():
            ...
            node_i = node_outputs.unsqueeze(2).expand(
                -1, -1, num_nodes, -1
            )  # (batch, nodes, nodes, output)
            node_j = node_outputs.unsqueeze(1).expand(
                -1, num_nodes, -1, -1
            )  # (batch, nodes, nodes, output)
            ...
            return {
                ...
                "max_risk_per_vessel": collision_risks.max(dim=2)[
                    0
                ],  # Max risk for each vessel
            }
...
class GCNLayer(nn.Module):
    ...
    def forward(
        ...
        for i in range(num_nodes):
            ...
            node_i_features = node_transformed[:, i : i + 1, :].expand(
                -1, num_nodes, -1
            )  # (batch, nodes, output_dim)

            # Edge features from i to all neighbors
            edge_i_features = self.edge_linear(
                edge_features[:, i, :, :]
            )  # (batch, nodes, output_dim)

            # Combine node and edge features
            combined_features = torch.cat(
                [node_i_features, edge_i_features], dim=-1
            )  # (batch, nodes, 2*output_dim)
            messages_i = self.message_linear(
                combined_features
            )  # (batch, nodes, output_dim)

            # Apply neighbor mask and aggregate
            messages_i = messages_i * neighbors_mask.unsqueeze(-1)  # Mask non-neighbors
            aggregated_message = messages_i.sum(
                dim=1, keepdim=True
            )  # (batch, 1, output_dim)
...
if __name__ == "__main__":
    ...
    model1 = create_baseline_model(
        "trajectory_prediction", input_dim=4, hidden_dim=64, output_dim=4
    )
    x1 = torch.randn(2, 10, 4)  # batch=2, seq=10, features=4
    out1 = model1(x1)
    print(f"TrajectoryLSTM output shape: {out1.shape}")

    # Test AnomalyAutoencoder
    print("\nTesting AnomalyAutoencoder...")
    model2 = create_baseline_model("anomaly_detection", input_dim=4, encoding_dim=32)
    x2 = torch.randn(2, 10, 4)  # batch=2, seq=10, features=4
    ...
    model3 = create_baseline_model(
        "vessel_interaction", node_features=10, edge_features=5, hidden_dim=32
    )
