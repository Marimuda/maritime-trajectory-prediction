...
from src.models.baseline_models import TrajectoryLSTM, AnomalyAutoencoder, VesselGCN, create_baseline_model
from src.models.metrics import (
    TrajectoryPredictionMetrics, AnomalyDetectionMetrics, VesselInteractionMetrics,
    MaritimeLossFunctions, create_metrics, create_loss_function
)
...
class BaselineTrainer:
    ...
    def __init__(
        ...
        logger.info(f"Initialized {task} trainer with {sum(p.numel() for p in model.parameters())} parameters")
    ...
    def _forward_pass(self, batch: List[torch.Tensor]) -> Tuple[torch.Tensor, Optional[torch.Tensor], Optional[torch.Tensor]]:
...
class TrajectoryTrainer(BaselineTrainer):
    ...
    def _forward_pass(self, batch: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
...
class InteractionTrainer(BaselineTrainer):
    ...
    def _forward_pass(self, batch: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        # Compute loss
        loss = MaritimeLossFunctions.interaction_loss(collision_predictions, collision_targets)
...
def load_and_prepare_data(
    ...
    test_data = TensorDataset(
        ...
        torch.FloatTensor(dataset['test']['y'])
    )
    ...
    logger.info(f"Data loaded: {len(train_data)} train, {len(val_data)} val, {len(test_data)} test samples")
