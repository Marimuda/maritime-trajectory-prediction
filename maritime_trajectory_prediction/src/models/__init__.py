...
def get_model_info(model_type: str, task: str = None) -> dict:
    ...
    if model_type == 'baseline':
        ...
        if task not in BASELINE_MODELS:
            raise ValueError(f"Unknown task: {task}. Available: {list(BASELINE_MODELS.keys())}")
        ...
        return {
            ...
            'description': model_class.__doc__.split('\n')[1].strip() if model_class.__doc__ else "No description available"
        }
...
def list_available_models() -> dict:
    ...
    # Baseline models
    for task in BASELINE_MODELS.keys():
...
def __getattr__(name: str):
    ...
    if name == 'lightning_models':
        try:
            ...
            raise ImportError("PyTorch Lightning not available. Install with: pip install pytorch-lightning")
...
def quick_evaluate(task: str, model_path: str, data_path: str, **kwargs):
    ...
    test_data = torch.randn(100, 10, 13)  # Dummy data
