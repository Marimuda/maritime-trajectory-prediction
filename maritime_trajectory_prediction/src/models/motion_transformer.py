...
class MotionDecoder(nn.Module):
    ...
    def forward(
        ...
        decoder_output = decoder_output.view(
            batch_size, self.prediction_horizon, self.n_queries, self.d_model
        )
...
class MotionTransformer(nn.Module):
    ...
    def compute_loss(
        ...
        trajectories = outputs[
            "trajectories"
        ]  # [batch_size, pred_horizon, n_queries, output_dim]
        ...
        if loss_type == "best_of_n":
            ...
            trajectory_errors = F.mse_loss(
                trajectories, targets_expanded, reduction="none"
            )
            trajectory_errors = trajectory_errors.mean(
                dim=(1, 3)
...
class MotionTransformerTrainer:
    ...
    def validate_step(
        ...
        with torch.no_grad():
            ...
            ade = (
                F.mse_loss(best_trajectory, targets, reduction="none")
                .mean(dim=-1)
                .sqrt()
                .mean()
            )
            fde = (
                F.mse_loss(best_trajectory[:, -1], targets[:, -1], reduction="none")
                .mean(dim=-1)
                .sqrt()
                .mean()
            )

            loss_dict.update({"val_ade": ade, "val_fde": fde})
...
if __name__ == "__main__":
    ...
    # Test input
    batch_size, context_len, input_dim = 4, 20, 4
    pred_horizon = 10
