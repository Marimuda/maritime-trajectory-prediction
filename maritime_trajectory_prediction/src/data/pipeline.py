...
from .schema import ColumnNames, get_required_columns
...
class DatasetConfig:
    ...
    temporal_bounds: dict[str, datetime] | None = None
...
class BaseDatasetBuilder(abc.ABC):
    ...
    def filter_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.config.spatial_bounds:
            ...
            spatial_mask = (
                (filtered_df[ColumnNames.LAT] >= bounds.get("min_lat", -90))
                & (filtered_df[ColumnNames.LAT] <= bounds.get("max_lat", 90))
                & (filtered_df[ColumnNames.LON] >= bounds.get("min_lon", -180))
                & (filtered_df[ColumnNames.LON] <= bounds.get("max_lon", 180))
            )
            filtered_df = filtered_df[spatial_mask]
            self.logger.info(
                f"Spatial filtering: {len(df)} -> {len(filtered_df)} records"
            )
        ...
        if self.config.temporal_bounds:
            ...
            self.logger.info(
                f"Temporal filtering: {len(df)} -> {len(filtered_df)} records"
            )
        ...
        if self.config.vessel_types and "shiptype" in filtered_df.columns:
            ...
            self.logger.info(
                f"Vessel type filtering: {len(df)} -> {len(filtered_df)} records"
            )
    ...
    def resample_trajectories(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for mmsi, vessel_df in df.groupby("mmsi"):
            try:
                ...
                if len(vessel_df) < self.config.min_trajectory_length:
                    self.logger.debug(
                        f"Skipping vessel {mmsi}: only {len(vessel_df)} records"
                    )
                    continue
                ...
                if len(resampled) >= self.config.min_trajectory_length:
                    ...
                    self.logger.debug(
                        f"Vessel {mmsi} dropped after resampling: {len(resampled)} records"
                    )
        ...
        if resampled_dfs:
            ...
            self.logger.info(
                f"Resampling: {df['mmsi'].nunique()} -> {result['mmsi'].nunique()} vessels"
            )
    ...
    def split_dataset(
        ...
        self.logger.info(
            f"Dataset splits - Train: {len(train_idx)}, Val: {len(val_idx)}, Test: {len(test_idx)}"
        )
    ...
    def build_dataset(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        self.metadata = DatasetMetadata(
            ...
            spatial_bounds={
                "min_lat": resampled_df[ColumnNames.LAT].min(),
                "max_lat": resampled_df[ColumnNames.LAT].max(),
                "min_lon": resampled_df[ColumnNames.LON].min(),
                "max_lon": resampled_df[ColumnNames.LON].max(),
            },
            feature_columns=list(features_df.columns),
            target_columns=list(targets_df.columns)
            if hasattr(targets_df, "columns")
            else ["target"],
        )
...
class DataPipeline:
    ...
    def register_builder(self, task: MLTask, builder_class: type):
        ...
        self.logger.info(
            f"Registered builder for {task.value}: {builder_class.__name__}"
        )

    def process_raw_data(self, file_path: str | Path) -> pd.DataFrame:
    ...
    def build_dataset(
        self, df: pd.DataFrame, task: MLTask, config: DatasetConfig
    ) -> dict[str, Any]:
    ...
    def export_dataset(
        ...
        output_dir: Path,
        formats: list[DatasetFormat] = None,
    ) -> dict[str, Path]:
        ...
        for format_type in formats:
            ...
            if format_type == DatasetFormat.PARQUET:
                ...
                for split_name, (X, y) in splits.items():
                    ...
                    exported_files[f"{format_type.value}_{split_name}_features"] = (
                        features_file
                    )
                    exported_files[f"{format_type.value}_{split_name}_targets"] = (
                        targets_file
                    )
    ...
    def load_dataset(
        self, dataset_dir: Path, format_type: DatasetFormat = DatasetFormat.NUMPY
    ) -> dict[str, Any]:
        ...
        self.logger.info(
            f"Loaded dataset from {dataset_dir} in {format_type.value} format"
        )

        return {"splits": splits, "metadata": metadata_dict}
