...
class MessageTypeConfig:
    ...
    # All supported types
    ALL_TYPES: Set[int] = CORE_POSITION_TYPES | CONTEXT_TYPES | METADATA_TYPES | SAFETY_TYPES
...
class FieldConfig:
    ...
    UNIVERSAL_FIELDS: Set[str] = frozenset({
        ...
        'message_type', 'rxtime', 'channel'
    })
    
    # Task-specific field groups
    TRAJECTORY_FIELDS: Set[str] = frozenset({
        'sog', 'cog', 'heading', 'turn', 'status', 
        'accuracy', 'second', 'maneuver', 'raim'
    })
    
    ANOMALY_FIELDS: Set[str] = frozenset({
        'shipname', 'shiptype', 'to_bow', 'to_stern', 
        'to_port', 'to_starboard', 'destination', 'eta', 'draught'
    })
    
    GRAPH_FIELDS: Set[str] = frozenset({
        'callsign', 'imo', 'aid_type', 'name', 
        'off_position', 'virtual_aid'
    })
    
    SAFETY_FIELDS: Set[str] = frozenset({
        'safety_text', 'urgency', 'response_required'
    })
...
class AISMultiTaskProcessor:
    ...
    def __init__(self, target_tasks: List[Union[MLTask, str]] = None):
        """
        ...
        """
        self.target_tasks = self._parse_tasks(target_tasks or [MLTask.TRAJECTORY_PREDICTION])
        ...
        logger.info(f"Initialized AIS Multi-Task Processor for tasks: {[t.value for t in self.target_tasks]}")
        logger.info(f"Required message types: {sorted(self.required_message_types)}")
    
    def _parse_tasks(self, tasks: List[Union[MLTask, str]]) -> List[MLTask]:
    ...
    def parse_line(self, line: str) -> Optional[Dict]:
        ...
        try:
            ...
            self.stats['message_type_counts'][msg_type] = self.stats['message_type_counts'].get(msg_type, 0) + 1
    ...
    def _extract_fields(self, data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            if any(task in [MLTask.ANOMALY_DETECTION, MLTask.PORT_OPERATIONS] for task in self.target_tasks):
                if 'shipname' in data:
                ...
                try:
                    ...
                    if len(timestamp_str) == 14:  # YYYYMMDDHHMMSS
                        ...
                        if (1900 <= year <= 2100 and 1 <= month <= 12 and 1 <= day <= 31 and
                            ...
                            record['time'] = pd.Timestamp(year, month, day, hour, minute, second, tz='UTC')
                        else:
                            record['time'] = pd.NaT
                    else:
                        record['time'] = pd.NaT
    ...
    def process_file(self, file_path: Union[str, Path], chunk_size: int = 10000) -> pd.DataFrame:
        ...
        with open(file_path, 'r') as f:
            for i, line in enumerate(f):
                ...
                # Progress logging
                if (i + 1) % chunk_size == 0:
                    logger.info(f"Processed {i + 1:,} lines, {len(records):,} valid records")
    ...
    def get_task_specific_dataset(self, df: pd.DataFrame, task: MLTask) -> pd.DataFrame:
        ...
        mask = df.apply(lambda row: self._is_relevant_for_task(row.to_dict(), task), axis=1)
        task_df = df[mask].copy()
        
        # Task-specific processing
        if task == MLTask.TRAJECTORY_PREDICTION:
            # Keep only position reports, sort by vessel and time
            task_df = task_df[task_df['msg_class'].isin(['A_pos', 'B_pos', 'long_range'])]
