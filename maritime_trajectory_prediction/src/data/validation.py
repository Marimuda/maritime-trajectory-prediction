...
class DataValidator:
    ...
    def _validate_structure(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        required_columns = ["mmsi", "time", "lat", "lon"]
        ...
        for col in numeric_columns:
            ...
            if len(col_data) > 0:
                ...
                if outlier_percentage > 5:
                    warnings.append(
                        f"High outlier percentage in {col}: {outlier_percentage:.1f}%"
                    )
        ...
        metrics.update(
            {
                ...
                "invalid_coordinates": (invalid_lat if "lat" in df.columns else 0)
                + (invalid_lon if "lon" in df.columns else 0),
    ...
    def _validate_task_specific(self, df: pd.DataFrame, task: str) -> dict[str, Any]:
        ...
        if task == "trajectory_prediction":
            ...
            if "mmsi" in df.columns and "time" in df.columns:
                time_groups = df.groupby(pd.Grouper(key="time", freq="10min"))[
                    "mmsi"
                ].nunique()
                sparse_windows = (time_groups < 2).sum()
                if sparse_windows > len(time_groups) * 0.5:
                    warnings.append(
                        f"Many sparse time windows for graph construction: {sparse_windows}"
                    )
...
class QualityChecker:
    ...
    def check_coverage_quality(self, df: pd.DataFrame) -> dict[str, Any]:
        ...
        if not all(col in df.columns for col in ["lat", "lon", "time"]):
    ...
    def export_to_parquet(self, dataset: dict[str, Any], name: str) -> dict[str, Path]:
        ...
        for split_name, (X, y) in splits.items():
            ...
            target_df = pd.DataFrame(
                y_reshaped, columns=[f"target_{i}" for i in range(y_reshaped.shape[1])]
            )
