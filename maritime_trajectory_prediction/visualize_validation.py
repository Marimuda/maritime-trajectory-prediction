...
def plot_performance_comparison(results):
    ...
    for bar, time in zip(bars, all_times):
    ...
    ax4.bar(model_names, param_counts, color=['#FF6B6B', '#4ECDC4', '#95E1D3', '#F38BA8', '#A8E6CF', '#FFD93D'])
    ...
    for i, v in enumerate(param_counts):
        ax4.text(i, v, f'{v:,}', ha='center', va='bottom', fontweight='bold', rotation=90)
...
def plot_scalability_analysis(results):
    ...
    for model_name, metrics in comp_results.items():
        if 'inference_times' in metrics:
            times = [metrics['inference_times'].get(str(bs), None) for bs in batch_sizes]
    ...
    ax1.set_title('Scalability: Inference Time vs Batch Size', fontsize=14, fontweight='bold')
    ax1.legend()
    ...
    for model_name, metrics in comp_results.items():
        if 'inference_times' in metrics:
            ...
            label = model_name.replace('_', ' ').title()
            ax2.plot(valid_batch_sizes_tp, throughputs, marker='s', linewidth=2, label=label)
    
    ax2.set_xlabel('Batch Size')
    ax2.set_ylabel('Throughput (samples/second)')
    ax2.set_title('Scalability: Throughput vs Batch Size', fontsize=14, fontweight='bold')
    ax2.legend()
...
def plot_efficiency_analysis(results):
    ...
    for model in models_ad:
        if model in comp_results:
            ...
            if 'sota' in model:
                detection_rates.append(ad_results['sota_anomaly_transformer']['detection_rate'])
            else:
                detection_rates.append(ad_results['baseline_autoencoder']['detection_rate'])
    
    colors_ad = ['#FF6B6B', '#4ECDC4']
    labels_ad = ['SOTA Anomaly Transformer', 'Baseline Autoencoder']
    
    for i, (params, rate, color, label) in enumerate(zip(params_ad, detection_rates, colors_ad, labels_ad)):
        ax1.scatter(params, rate, s=200, c=color, alpha=0.7, label=label)
        ax1.annotate(label, (params, rate), xytext=(10, 10), 
                    textcoords='offset points', fontsize=10, fontweight='bold')
    
    ax1.set_xlabel('Model Parameters')
    ax1.set_ylabel('Detection Rate')
    ax1.set_title('Anomaly Detection: Performance vs Complexity', fontsize=14, fontweight='bold')
    ax1.set_xscale('log')
    ax1.grid(True, alpha=0.3)
    ax1.legend()
    ...
    for i, (params, ade, color, label) in enumerate(zip(params_tp, ade_values, colors_tp, labels_tp)):
        ax2.scatter(params, ade, s=200, c=color, alpha=0.7, label=label)
        ax2.annotate(label, (params, ade), xytext=(10, 10), 
                    textcoords='offset points', fontsize=10, fontweight='bold')
    
    ax2.set_xlabel('Model Parameters')
    ax2.set_ylabel('Average Displacement Error (ADE)')
    ax2.set_title('Trajectory Prediction: Performance vs Complexity', fontsize=14, fontweight='bold')
    ax2.set_xscale('log')
    ax2.grid(True, alpha=0.3)
    ax2.legend()
...
def create_summary_dashboard(results):
    ...
    # Title
    fig.suptitle('SOTA Maritime Models Validation Dashboard', fontsize=20, fontweight='bold', y=0.95)
    ...
    model_info = [
        ...
        ['Anomaly Transformer (Small)', '3.2M', 'Anomaly Detection', '100% Detection Rate'],
    ...
    table = ax1.table(cellText=model_info[1:], colLabels=model_info[0], 
    ...
    metrics_data = {
        'SOTA Models': [100, 63.99, 0.16, 0.33],  # Detection rate %, ADE, AD time, TP time
    ...
    ax2.bar(x - width/2, metrics_data['SOTA Models'], width, label='SOTA Models', color='#FF6B6B')
    ax2.bar(x + width/2, metrics_data['Baseline Models'], width, label='Baseline Models', color='#4ECDC4')
    
    ax2.set_title('Key Performance Metrics', fontsize=14, fontweight='bold')
    ax2.set_xticks(x)
    ax2.set_xticklabels(['Detection Rate\n(%)', 'ADE\n(units)', 'AD Inference\n(s)', 'TP Inference\n(s)'])
    ax2.legend()
    ax2.set_yscale('log')
    ...
    ax3.bar(x - 0.2, param_counts_norm, 0.4, label='Parameters (normalized)', color='#95E1D3')
    ax3.bar(x + 0.2, inference_times_norm, 0.4, label='Inference Time (normalized)', color='#F38BA8')
    ...
    key_models = ['anomaly_transformer_small', 'motion_transformer_small', 
                  'baseline_autoencoder', 'baseline_lstm']
    colors = ['#FF6B6B', '#4ECDC4', '#95E1D3', '#F38BA8']
    
    for model_name, color in zip(key_models, colors):
        if model_name in comp_results:
            ...
            label = model_name.replace('_', ' ').title()
            ax4.plot(valid_batch_sizes, throughputs, marker='o', linewidth=2, 
    ...
    ax5.text(0.05, 0.9, '\n'.join(insights), transform=ax5.transAxes, 
            fontsize=12, verticalalignment='top', fontweight='bold')
    ax5.set_title('Key Insights & Recommendations', fontsize=14, fontweight='bold')
    ax5.axis('off')
    
    plt.savefig(os.path.join(PLOTS_DIR, 'validation_dashboard.png'), dpi=300, bbox_inches='tight')
    plt.close()
