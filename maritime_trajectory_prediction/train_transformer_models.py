...
class SOTATrainer:
    ...
    def __init__(self, config: SOTATrainingConfig):
        ...
        logger.info(f"Initialized SOTATrainer with {self.config.get('model.type')} model")
        ...
        for epoch in range(max_epochs):
            ...
            for batch_idx, batch in enumerate(train_loader):
                ...
                if batch_idx % log_freq == 0:
                    logger.info(f"Epoch {epoch}, Batch {batch_idx}, Loss: {loss_dict['total_loss']:.4f}")
                    ...
                    if self.config.get('logging.use_wandb'):
                        wandb.log({
                            ...
                            **{f'train_{k}': v for k, v in loss_dict.items() if k != 'total_loss'}
                        })
    ...
    def _validate(self, val_loader):
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_losses.append(loss_dict.get('total_loss', loss_dict.get('val_total_loss', 0)))
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Train SOTA models for maritime trajectory prediction')
    parser.add_argument('--config', type=str, help='Path to configuration file')
    parser.add_argument('--model-type', type=str, choices=['anomaly_transformer', 'motion_transformer', 'baseline'],
                       help='Model type to train')
    parser.add_argument('--task', type=str, choices=['trajectory_prediction', 'anomaly_detection', 'vessel_interaction'],
