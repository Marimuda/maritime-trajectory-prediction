...
class TestDataLoadingPerformance:
    ...
    def test_large_csv_loading_performance(self, tmp_path, ais_data_factory, benchmark):
        ...
        large_dataset = ais_data_factory(
            n_vessels=1000, n_points_per_vessel=1000, seed=42
        )
...
class TestDataProcessingPerformance:
    ...
    def test_sequence_generation_performance(self, ais_data_factory, benchmark):
        ...
        large_trajectory = ais_data_factory(
            n_vessels=1, n_points_per_vessel=10000, seed=42
        )
        processor = AISProcessor()
...
class TestMemoryUsagePerformance:
    ...
    def test_memory_usage_large_dataset(self, ais_data_factory):
        ...
        # Act
        large_dataset = ais_data_factory(
            n_vessels=500, n_points_per_vessel=2000, seed=42
        )
    ...
    def test_memory_efficiency_sequence_generation(self, ais_data_factory):
        ...
        trajectory_data = ais_data_factory(
            n_vessels=10, n_points_per_vessel=1000, seed=42
        )
        ...
        sequences = processor.get_trajectory_sequences(
            trajectory_data, sequence_length=100, prediction_horizon=20
        )
...
class TestScalabilityPerformance:
    ...
    def test_processing_time_scaling(self, ais_data_factory):
        ...
        for size in data_sizes:
            ...
            test_data = ais_data_factory(
                n_vessels=10, n_points_per_vessel=size, seed=42
            )
        ...
        time_ratios = [
            processing_times[i + 1] / processing_times[i]
            for i in range(len(processing_times) - 1)
        ]
        size_ratios = [
            data_sizes[i + 1] / data_sizes[i] for i in range(len(data_sizes) - 1)
        ]
    ...
    def test_concurrent_processing_performance(self, ais_data_factory):
        ...
        def process_data():
            processor = AISProcessor()
            start_time = time.time()
            processor.clean_ais_data(test_data.copy())  # Don't store unused result
            processing_time = time.time() - start_time
...
class TestRegressionBenchmarks:
    ...
    def test_baseline_processing_performance(self, ais_data_factory, benchmark):
        ...
        # Arrange
        baseline_data = ais_data_factory(n_vessels=50, n_points_per_vessel=200, seed=42)
    ...
    @pytest.mark.perf
    def test_baseline_trajectory_extraction_performance(
        self, ais_data_factory, benchmark
    ):
        """Establish baseline for trajectory extraction performance."""
        # Arrange
        baseline_data = ais_data_factory(n_vessels=20, n_points_per_vessel=100, seed=42)
