...
from src.data.maritime_message_processor import (
    AISProcessor,
    load_ais_data,
    preprocess_ais_data,
)
...
class TestAISProcessor:
    ...
    def sample_dataframe(self):
        ...
        data = {
            ...
            "lat": [62.006901, 62.006180, 61.966694],
            "lon": [-6.774024, -6.772121, -6.492732],
            "sog": [0.0, 0.0, 9.2],
            "cog": [278.8, 0.0, 15.3],
            "time": pd.to_datetime(
                ["2025-05-08 10:10:19", "2025-05-08 10:10:20", "2025-05-08 10:10:21"],
                utc=True,
            ),
    ...
    def test_parse_valid_line(self, processor, sample_ais_line):
        ...
        assert "lat" in result  # CF-compliant field name
        assert "lon" in result  # CF-compliant field name
    ...
    def test_parse_invalid_lines(self, processor, invalid_lines):
        ...
        assert processor.stats["filtered_records"] + processor.stats[
            "error_records"
        ] == len(invalid_lines)
    ...
    def test_field_mapping(self, processor):
        ...
        # Original fields 'speed' and 'course' should be mapped to 'sog' and 'cog'
    ...
    def test_sentinel_value_handling(self, processor):
        ...
        assert result is not None
        assert pd.isna(result["lat"])  # lat=91 should become NaN
        assert pd.isna(result["lon"])  # lon=181 should become NaN
    ...
    def test_clean_ais_data(self, processor, sample_dataframe):
        ...
        assert not cleaned_df["lat"].isna().any()
        assert not cleaned_df["lon"].isna().any()
        assert (
            cleaned_df["mmsi"]
            .apply(
                lambda x: (100_000_000 <= x <= 799_999_999)
                or (2_000_000 <= x <= 9_999_999)
                or (990_000_000 <= x <= 999_999_999)
            )
            .all()
        )
