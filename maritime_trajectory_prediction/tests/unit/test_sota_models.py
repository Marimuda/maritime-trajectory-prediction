...
class TestAnomalyTransformer:
    ...
    def sample_data(self):
        ...
        batch_size, seq_len, input_dim = 4, 20, 13
    ...
    def test_anomaly_transformer_forward(self, anomaly_transformer, sample_data):
        ...
        required_keys = ['reconstruction', 'anomaly_scores', 'association_discrepancy', 'attention_weights']
        ...
        # Check value ranges
        assert (outputs['anomaly_scores'] >= 0).all() and (outputs['anomaly_scores'] <= 1).all()
    ...
    def test_anomaly_criterion_computation(self, anomaly_transformer, sample_data):
        ...
        required_keys = ['reconstruction_loss', 'association_loss', 'anomaly_criterion', 'total_loss']
        for key in required_keys:
        ...
        for key in ['reconstruction_loss', 'association_loss', 'anomaly_criterion', 'total_loss']:
    ...
    def test_anomaly_detection(self, anomaly_transformer, sample_data):
        ...
        # Check result keys
        required_keys = ['anomaly_scores', 'binary_anomalies', 'reconstruction_error', 
                        'association_discrepancy', 'confidence']
        for key in required_keys:
        ...
        assert ((results['binary_anomalies'] == 0) | (results['binary_anomalies'] == 1)).all()
...
class TestMotionTransformer:
    ...
    def sample_context(self):
        ...
        batch_size, context_len, input_dim = 4, 15, 13
    ...
    def test_context_encoder(self):
        ...
        encoder = ContextEncoder(input_dim=13, d_model=128, n_layers=2)
        
        batch_size, seq_len, input_dim = 2, 15, 13
    ...
    def test_motion_decoder(self):
        ...
        # Check shapes
        assert outputs['trajectories'].shape == (batch_size, 10, 4, 4)  # pred_horizon, n_queries, output_dim
        ...
        assert (outputs['confidences'] >= 0).all() and (outputs['confidences'] <= 1).all()
    ...
    def test_motion_transformer_forward(self, motion_transformer, sample_context):
        ...
        required_keys = ['trajectories', 'confidences', 'context_features', 'query_features']
        for key in required_keys:
            assert key in outputs
    ...
    def test_loss_computation(self, motion_transformer, sample_context, sample_targets):
        ...
        # Test best-of-N loss
        loss_dict = motion_transformer.compute_loss(outputs, sample_targets, 'best_of_n')
        
        expected_keys = ['total_loss', 'regression_loss', 'classification_loss', 'best_mode_errors']
        for key in expected_keys:
            assert key in loss_dict
        ...
        # Test weighted loss
        loss_dict_weighted = motion_transformer.compute_loss(outputs, sample_targets, 'weighted')
        
        expected_keys_weighted = ['total_loss', 'regression_loss', 'confidence_regularization']
...
class TestSOTAModelIntegration:
    ...
    def test_model_creation_api(self):
        ...
        anomaly_model = create_model('anomaly_transformer', input_dim=13, d_model=128)
        assert isinstance(anomaly_model, AnomalyTransformer)
        
        # Test Motion Transformer creation
        motion_model = create_model('motion_transformer', input_dim=13, d_model=128)
    ...
    def test_model_parameter_counts(self):
        ...
        anomaly_model = create_anomaly_transformer(
            input_dim=13, d_model=128, n_heads=4, n_layers=2
        )
        motion_model = create_motion_transformer(
            input_dim=13, d_model=128, n_queries=4, encoder_layers=2, decoder_layers=3
        )
    ...
    def test_model_device_compatibility(self):
        ...
        anomaly_model = create_anomaly_transformer(input_dim=13, d_model=64, n_layers=1)
        anomaly_model = anomaly_model.to(device)
        
        x = torch.randn(2, 10, 13).to(device)
        outputs = anomaly_model(x)
        assert outputs['reconstruction'].device.type == device.split(':')[0]
        
        # Test Motion Transformer
        motion_model = create_motion_transformer(
            input_dim=13, d_model=64, encoder_layers=1, decoder_layers=1, prediction_horizon=5
        )
        motion_model = motion_model.to(device)
        
        context = torch.randn(2, 10, 13).to(device)
    ...
    def test_model_gradient_flow(self):
        ...
        # Test Anomaly Transformer
        anomaly_model = create_anomaly_transformer(input_dim=13, d_model=64, n_layers=1)
        x = torch.randn(2, 10, 13, requires_grad=True)
        ...
        context = torch.randn(2, 10, 13, requires_grad=True)
...
class TestSOTAModelPerformance:
    ...
    def test_inference_speed(self):
        ...
        anomaly_model = create_anomaly_transformer(input_dim=13, d_model=128, n_layers=2)
        motion_model = create_motion_transformer(
            input_dim=13, d_model=128, encoder_layers=2, decoder_layers=2, prediction_horizon=10
        )
        ...
        # Test data
        x = torch.randn(8, 20, 13)  # Batch of 8 sequences
    ...
    def test_memory_usage(self):
        ...
        # Create models
        anomaly_model = create_anomaly_transformer(input_dim=13, d_model=256, n_layers=4)
        motion_model = create_motion_transformer(
            input_dim=13, d_model=256, encoder_layers=3, decoder_layers=4
        )
        ...
        # Test forward pass
        x = torch.randn(16, 50, 13)  # Larger batch for memory testing
