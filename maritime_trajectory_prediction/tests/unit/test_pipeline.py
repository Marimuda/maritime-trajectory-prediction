...
from src.data.multi_task_processor import AISMultiTaskProcessor, MLTask
...
class TestAnomalyDetectionBuilder:
            ...
            data.append(
                {
                    ...
                    "status": 0,
                    "shiptype": 30,
                    "message_type": 1,  # Class A position report
                    "msg_class": "A_pos",  # Post-pipeline format
                }
            )
    ...
    def config(self):
        """Create test configuration."""
        return DatasetConfig(
            task=MLTask.ANOMALY_DETECTION, sequence_length=5, min_trajectory_length=10
        )
...
class TestDatasetExporter:
    ...
    def sample_dataset(self):
        ...
        return {"splits": splits, "metadata": metadata}
...
class TestDataPipeline:
    ...
    def test_register_builder(self, mock_processor):
        ...
        pipeline = DataPipeline(mock_processor)
        pipeline.register_builder(
            MLTask.TRAJECTORY_PREDICTION, TrajectoryPredictionBuilder
        )

        assert MLTask.TRAJECTORY_PREDICTION in pipeline.builders
        assert (
            pipeline.builders[MLTask.TRAJECTORY_PREDICTION]
            == TrajectoryPredictionBuilder
        )
    ...
    def test_build_dataset(self, mock_processor):
        ...
        dataset = pipeline.build_dataset(
            sample_data, MLTask.TRAJECTORY_PREDICTION, config
        )
    ...
    def test_export_dataset(self, mock_processor):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            exported_files = pipeline.export_dataset(
                dataset, Path(temp_dir), formats=[DatasetFormat.NUMPY]
            )
...
@pytest.fixture
def real_ais_data():
    """Create realistic AIS data for integration testing - matches POST-PIPELINE format."""
    np.random.seed(42)
    data = []
    base_time = datetime(2024, 1, 1)
    ...
    for vessel in vessel_configs:
        for i in range(100):
            ...
            lon_change = (
                distance_nm
                * np.sin(np.radians(vessel["course"]))
                / (60 * np.cos(np.radians(vessel["start_lat"])))
            )

            # Map message type to message class (post-pipeline format)
            msg_type = vessel["msg_type"]
            if msg_type in [1, 2, 3]:
                msg_class = "A_pos"
            elif msg_type in [18, 19]:
                msg_class = "B_pos"
            elif msg_type == 4:
                msg_class = "base_station"
            elif msg_type == 5:
                msg_class = "static_voyage"
            elif msg_type == 27:
                msg_class = "long_range"
            else:
                msg_class = "other"
...
class TestIntegration:
    ...
    def test_end_to_end_trajectory_prediction(self, real_ais_data):
        ...
        # Build dataset
        dataset = pipeline.build_dataset(
            real_ais_data, MLTask.TRAJECTORY_PREDICTION, config
        )
    ...
    def test_end_to_end_anomaly_detection(self, real_ais_data):
        ...
        anomaly_data.loc[100, "cog"] = (
            anomaly_data.loc[99, "cog"] + 180
        )  # Course anomaly
        ...
        # Create configuration
        config = DatasetConfig(
            task=MLTask.ANOMALY_DETECTION, sequence_length=5, min_trajectory_length=20
        )
    ...
    def test_export_import_cycle(self, real_ais_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            np.testing.assert_array_almost_equal(
                original_train_X, loaded_train_X, decimal=6
            )
            np.testing.assert_array_almost_equal(
                original_train_y, loaded_train_y, decimal=6
            )
