...
class TestMaritimeUtils:
    ...
    def test_validate_trajectory_valid_data(self):
        ...
        data = {
            'latitude': [62.0, 62.01, 62.02],
            'longitude': [-6.7, -6.69, -6.68],
        ...
        assert not validated_df['calculated_speed'].iloc[1:].isna().all()  # Should have calculated speeds
    
    def test_validate_trajectory_unrealistic_speed(self):
        """Test trajectory validation with unrealistic speeds."""
        data = {
            'latitude': [62.0, 63.0],  # Large jump
            'longitude': [-6.7, -5.0],  # Large jump
            'time': pd.to_datetime(['2025-05-08 10:00:00', '2025-05-08 10:01:00'], utc=True)  # 1 minute
        }
        df = pd.DataFrame(data)
