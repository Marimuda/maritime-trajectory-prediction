...
class TestTrainingPipeline:
    ...
    def test_model_forward_pass(self, temp_data_file):
        ...
        from src.models.motion_transformer import create_motion_transformer, MARITIME_MTR_CONFIG
    ...
    def test_feature_alignment(self, temp_data_file):
        ...
        print(f"Input tensor - min/max: {input_tensor.min():.3f}/{input_tensor.max():.3f}")
        print(f"Input tensor - mean/std: {input_tensor.mean():.3f}/{input_tensor.std():.3f}")
        
        print(f"Target tensor - shape: {target_tensor.shape}")
        print(f"Target tensor - min/max: {target_tensor.min():.3f}/{target_tensor.max():.3f}")
        print(f"Target tensor - mean/std: {target_tensor.mean():.3f}/{target_tensor.std():.3f}")
