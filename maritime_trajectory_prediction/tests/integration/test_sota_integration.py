...
class TestSOTADataIntegration:
    ...
    def processed_sequences(self, sample_ais_data):
        ...
        for i in range(len(data) - sequence_length - prediction_horizon + 1):
            ...
            target_tensor = torch.tensor(
                target[target_cols].values, dtype=torch.float32
            )
    ...
    def test_anomaly_transformer_with_ais_data(self, processed_sequences):
        ...
        assert results["binary_anomalies"].shape == (
            sequences.size(0),
            sequences.size(1),
        )
    ...
    def test_models_with_variable_sequence_lengths(self, sample_ais_data):
        ...
        for seq_len in [10, 15, 25]:
            ...
            seq_tensor = torch.tensor(
                seq[feature_cols].values, dtype=torch.float32
            ).unsqueeze(0)
            ...
            assert (
                motion_outputs["trajectories"].shape[1] == 10
            )  # prediction_horizon (small config)
...
class TestSOTATrainingIntegration:
    ...
    def training_data(self):
        ...
        batch_size, seq_len, input_dim = 16, 20, 4
    ...
    def test_training_convergence(self):
        ...
        batch_size, seq_len, input_dim = 8, 15, 4
        ...
        targets = context[:, -1:, :4].repeat(1, pred_horizon, 1) + 0.1 * torch.randn(
            batch_size, pred_horizon, 4
        )
...
class TestSOTAModelComparison:
    ...
    def test_model_complexity_comparison(self):
        ...
        # Create models
        baseline_lstm = create_baseline_model("trajectory_prediction", input_dim=4)
        baseline_autoencoder = create_baseline_model("anomaly_detection", input_dim=4)
        anomaly_transformer = create_model(
            "anomaly_transformer", input_dim=4, d_model=128, n_layers=2
        )
        motion_transformer = create_model(
            "motion_transformer",
            input_dim=4,
            d_model=128,
            encoder_layers=2,
    ...
    def test_inference_time_comparison(self):
        ...
        context = torch.randn(batch_size, seq_len, 4)
