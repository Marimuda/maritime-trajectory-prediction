...
2025-05-08 11:10:20,199 - [AIS engine v0.61 #0-0]                 received: 6 msgs, total: 2778 msgs, rate: 1.99595 msg/s"""
    ...
    def test_end_to_end_processing(self, sample_log_content):
        ...
        try:
            ...
            position_data = cleaned_df[cleaned_df['msg_class'].str.contains('pos', na=False)]
            ...
            for mmsi, vessel_data in position_data.groupby('mmsi'):
                ...
                if 'sog' in vessel_data.columns:
                    ...
                    if speeds:
                        ...
                        assert behavior in ['anchored', 'maneuvering', 'transit', 'high_speed', 'unknown']
    ...
    def test_real_data_processing(self):
        ...
        assert stats['lines_processed'] > stats['valid_records']  # Some filtering should occur
        ...
        if 'longitude' in cleaned_df.columns and not cleaned_df['longitude'].isna().all():
...
class TestDataQualityAssurance:
    ...
    def test_mmsi_validation_comprehensive(self):
        ...
        for mmsi, should_be_valid, description in test_cases:
            ...
            if should_be_valid:
                assert result is not None, f"Valid MMSI {mmsi} ({description}) should be accepted"
                assert result['mmsi'] == mmsi
            else:
                assert result is None, f"Invalid MMSI {mmsi} ({description}) should be rejected"
    ...
    def test_temporal_data_validation(self):
        ...
        for timestamp, should_be_valid in test_cases:
            ...
            if should_be_valid:
                assert result is not None, f"Valid timestamp {timestamp} should be accepted"
                assert 'time' in result
                assert isinstance(result['time'], pd.Timestamp)
            else:
                assert result is None, f"Invalid timestamp {timestamp} should be rejected"
