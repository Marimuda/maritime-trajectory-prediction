...
def test_imports():
    ...
    try:
        ...
        print(
            f"✓ Main package imported successfully (version: {maritime_trajectory_prediction.__version__})"
        )
    ...
    try:
        ...
        json_str = json.dumps(
            test_data, default=lambda x: float(x) if isinstance(x, np.number) else x
        )
