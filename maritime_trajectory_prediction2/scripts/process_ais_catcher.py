...
def process_static_data(static_df):
    ...
    if not type5_df.empty:
        ...
        type5_latest = (
            type5_df.sort_values("timestamp").groupby("mmsi").last().reset_index()
        )
...
def create_trajectories(
    position_df, max_gap_minutes=MAX_GAP_MINUTES, min_points=MIN_POINTS, logger=None
):
    """
    ...
    for mmsi, group in tqdm(
        grouped, desc="Creating vessel trajectories", total=num_vessels
    ):
        # Sort by timestamp
        ...
        if len(gap_indices) == 0:
            ...
            if len(group) >= min_points:
                ...
                trajectory_stats["max_points"] = max(
                    trajectory_stats["max_points"], len(group)
                )
                trajectory_stats["min_points"] = min(
                    trajectory_stats["min_points"], len(group)
                )
        else:
            ...
            for gap_idx in gap_indices:
                ...
                if len(segment) >= min_points:
                    ...
                    trajectory_stats["max_points"] = max(
                        trajectory_stats["max_points"], len(segment)
                    )
                    trajectory_stats["min_points"] = min(
                        trajectory_stats["min_points"], len(segment)
                    )
                start_idx = gap_idx
        ...
        if vessel_segments > 0:
            ...
            if vessel_segments > 5:
                logger.debug(
                    f"Vessel MMSI {mmsi} has {vessel_segments} trajectory segments"
                )

    # Calculate statistics
    if trajectory_stats["trajectory_lengths"]:
        trajectory_stats["avg_points"] = sum(
            trajectory_stats["trajectory_lengths"]
        ) / len(trajectory_stats["trajectory_lengths"])
    else:
    ...
    logger.info(
        f"Created {len(trajectories)} trajectories from {trajectory_stats['vessels_with_trajectories']} vessels"
    )
    logger.info(
        f"Trajectory statistics: {trajectory_stats['vessels_with_trajectories']}/{trajectory_stats['total_vessels']} vessels have trajectories"
    )
    logger.info(
        f"Points per trajectory: min={trajectory_stats['min_points']}, max={trajectory_stats['max_points']}, avg={trajectory_stats['avg_points']:.1f}"
    )
    logger.info(
        f"Average segments per vessel: {trajectory_stats['total_segments']/max(1, trajectory_stats['vessels_with_trajectories']):.1f}"
    )
...
def add_derived_features(trajectory):
    ...
    traj.loc[mask, "course_drift"] = (
        traj.loc[mask, "heading"] - traj.loc[mask, "cog"]
    ).abs()
    traj.loc[traj["course_drift"] > 180, "course_drift"] = (
        360 - traj.loc[traj["course_drift"] > 180, "course_drift"]
    )

    # Calculate bearing deviation (difference between bearing and course)
    traj["bearing_deviation"] = None
    mask = ~traj["bearing"].isna()
    traj.loc[mask, "bearing_deviation"] = (
        traj.loc[mask, "bearing"] - traj.loc[mask, "cog"]
    ).abs()
    traj.loc[traj["bearing_deviation"] > 180, "bearing_deviation"] = (
        360 - traj.loc[traj["bearing_deviation"] > 180, "bearing_deviation"]
    )
...
def export_to_formats(trajectories, output_prefix):
    ...
    with open(jsonl_path, "w") as f:
        for traj in trajectories:
            ...
            vessel_obj = {"mmsi": mmsi, "static": static_data, "trajectory": traj_data}
...
def plot_trajectories(trajectories, output_file=None, max_trajectories=10):
    ...
    for i, traj in enumerate(trajectories_to_plot):
        ...
        vessel_name = (
            traj["name"].iloc[0]
            if "name" in traj.columns and not pd.isna(traj["name"].iloc[0])
            else f"MMSI: {mmsi}"
        )
        ...
        ax.plot(
            traj["lon"].iloc[0], traj["lat"].iloc[0], "o", color=colors[i], markersize=8
        )
        ax.plot(
            traj["lon"].iloc[-1],
            traj["lat"].iloc[-1],
...
def main():
    parser = argparse.ArgumentParser(
        description="Process AIS-catcher log files for maritime trajectory prediction"
    )
    parser.add_argument("log_file", help="Path to AIS-catcher log file")
    parser.add_argument(
        "--output", "-o", default="processed_ais", help="Output file prefix"
    )
    parser.add_argument(
        "--min-points",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Maximum gap in minutes between points",
    )
    parser.add_argument(
        "--visualize", action="store_true", help="Visualize trajectories"
    )
    parser.add_argument(
        "--max-trajectories",
        type=int,
        default=10,
        help="Maximum trajectories to visualize",
    )
    parser.add_argument(
    ...
    logger.info("Starting AIS data processing pipeline")
    ...
    position_df, static_df = parse_ais_catcher_log(
        args.log_file, logger=logger, max_records=args.max_records
    )
    ...
    logger.info(
        f"Adding derived features and filtering outliers for {len(trajectories)} trajectories"
    )
    ...
    for i, traj in enumerate(tqdm(trajectories, desc="Processing trajectories")):
        ...
        if len(filtered_traj) < len(traj_with_features) * 0.9:  # More than 10% filtered
            logger.debug(
                f"Trajectory {i}: {len(traj_with_features) - len(filtered_traj)} outliers filtered out "
