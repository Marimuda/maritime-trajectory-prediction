...
class AISDataProcessor:
    ...
    def parse_ais_message(self, message: dict) -> dict | None:
        """
    ...
    def save_to_zarr(
        self, dataset: xr.Dataset, mode: str = "w", consolidated: bool = True
    ) -> None:
        """
        ...
        # Save to Zarr
        dataset.to_zarr(
            self.zarr_path, mode=mode, consolidated=consolidated, encoding=encoding
        )
    ...
    def create_temporal_split(
        self, dataset: xr.Dataset, train_end: str, val_days: int = 7, test_days: int = 7
        ...
        logger.info(
            f"Created temporal splits: train={len(train_ds.time)}, "
    ...
    def optimize_chunks(
        self, dataset: xr.Dataset, target_chunk_size_mb: int = 128
    ) -> dict[str, int]:
        """
        ...
        optimized_chunks = {"time": time_chunk, "mmsi": mmsi_chunk}
