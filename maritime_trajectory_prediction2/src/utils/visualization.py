...
class TrajectoryVisualizer:
    ...
    def plot_trajectory(
        ...
        elif "distance_km" in trajectory.columns:
            ax2.plot(
                range(len(trajectory)), trajectory["distance_km"], "g-", linewidth=2
            )
    ...
    def plot_prediction_comparison(
        ...
        input_sequence: pd.DataFrame | None = None,
        ...
        if input_sequence is not None:
            ...
            ax.scatter(
                input_sequence["lon"].iloc[-1],
                input_sequence["lat"].iloc[-1],
                c="black",
                s=100,
                marker="o",
        ...
        ax.scatter(
            true_trajectory["lon"].iloc[0],
            true_trajectory["lat"].iloc[0],
            c="green",
            s=80,
            marker="o",
        )
        ax.scatter(
            true_trajectory["lon"].iloc[-1],
            true_trajectory["lat"].iloc[-1],
            c="green",
            s=80,
            marker="s",
        )
        ...
        ax.plot(
            predicted_trajectory["lon"],
            predicted_trajectory["lat"],
            "r--",
            linewidth=2,
            alpha=0.8,
        ...
        ax.scatter(
            predicted_trajectory["lon"].iloc[0],
            predicted_trajectory["lat"].iloc[0],
            c="red",
            s=80,
            marker="o",
        )
        ax.scatter(
            predicted_trajectory["lon"].iloc[-1],
            predicted_trajectory["lat"].iloc[-1],
            c="red",
            s=80,
            marker="s",
        )
    ...
    def create_interactive_map(
        ...
        for i, (traj, label) in enumerate(zip(trajectories, labels, strict=False)):
            ...
            folium.PolyLine(
                coordinates, color=color, weight=3, opacity=0.8, popup=label
            ).add_to(m)
